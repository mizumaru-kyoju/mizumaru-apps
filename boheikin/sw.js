// sw.js — 母平均（回転数・TY）区間推定アプリ用 Service Worker
// スコープ：/boheikin/ 配下のみ

const CACHE_VERSION = 'boheikin-v1';
...
const PRECACHE_URLS = [
  ...
  '/boheikin/index.html',
  '/boheikin/guide.html',
  '/boheikin/manifest.json',
  '/boheikin/logo.png',
  '/boheikin/icon-192.png',
  '/boheikin/icon-512.png',
  '/boheikin/icon-maskable-192.png',
  '/boheikin/icon-maskable-512.png'
];
  ...
  if (req.mode === 'navigate') {
    event.respondWith(
      fetch(req)
        ...
        .catch(() =>
          caches.match(req).then((cached) => cached || caches.match('/boheikin/index.html'))
        )
    );
    return;
  }
